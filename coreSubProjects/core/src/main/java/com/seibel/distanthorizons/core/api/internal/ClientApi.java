...
public class ClientApi
{
	...
	/** Delay before clearing render cache after level load to ensure chunks have loaded */
	private Timer postLevelLoadCacheClearTimer;
	private static final long POST_LEVEL_LOAD_CACHE_CLEAR_DELAY_IN_MS = 3_000;
	...
	{
		// only continue if the client is connected to a remote server (dedicated or LAN guest)
		boolean connectedToServer = MC_CLIENT.clientConnectedToRemoteServer();
		boolean connectedToReplay = MC_CLIENT.connectedToReplay();
		boolean hasSinglePlayerServer = MC_CLIENT.hasSinglePlayerServer();
		boolean connectedToDedicatedServer = MC_CLIENT.clientConnectedToDedicatedServer();

		LOGGER.info("onClientOnlyConnected called: connectedToServer=[{}], connectedToReplay=[{}], hasSinglePlayerServer=[{}], connectedToDedicatedServer=[{}]",
			connectedToServer, connectedToReplay, hasSinglePlayerServer, connectedToDedicatedServer);
	...
	public synchronized void onClientOnlyDisconnected()
	{
		...
		// clear the post-load cache clear timer
		if (this.postLevelLoadCacheClearTimer != null)
		{
			this.postLevelLoadCacheClearTimer.cancel();
			this.postLevelLoadCacheClearTimer = null;
		}
	...
	public void clientLevelLoadEvent(IClientLevelWrapper levelWrapper)
	{
		...
		if (MC_CLIENT.clientConnectedToRemoteServer())
		{
			if (this.firstLevelLoadTimer == null)
			{
		...
		try
		{
			...
			if (world != null)
			{
				...
				// Schedule a delayed cache clear to ensure LODs render after chunks have loaded.
				// This fixes the issue where LODs don't appear until settings are changed,
				// caused by the renderer starting before chunk data is available.
				this.schedulePostLevelLoadCacheClear();
			}
			else
			{
	...
	/**
	 * Schedules a delayed render cache clear after level load.
	 * This ensures LODs appear after the initial chunk batch has loaded,
	 * fixing the issue where LODs don't render until settings are changed.
	 */
	private void schedulePostLevelLoadCacheClear()
	{
		...
		this.postLevelLoadCacheClearTimer = TimerUtil.CreateTimer("PostLevelLoadCacheClearTimer");
		this.postLevelLoadCacheClearTimer.schedule(new TimerTask()
		{
			@Override
			public void run()
			{
				LOGGER.info("Executing post-level-load render cache clear to ensure LODs are visible.");
				DhApiRenderProxy.INSTANCE.clearRenderDataCache();
			}
		}, POST_LEVEL_LOAD_CACHE_CLEAR_DELAY_IN_MS);
	}
