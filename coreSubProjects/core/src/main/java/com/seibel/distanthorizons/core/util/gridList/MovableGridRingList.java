...
public class MovableGridRingList<T> extends ArrayList<T> implements List<T>
{
	...
	public void clear(Consumer<? super T> removedItemConsumer)
	{
		...
		try
		{
			...
			super.clear();
			// Pre-populate with nulls so indexed get/set operations work
			// Using addAll with nCopies is O(1) arraycopy vs O(n) individual adds
			super.addAll(Collections.nCopies(this.width * this.width, null));
		}
		finally
		{
