...
public class LodRenderer
{
	...
	private void renderLodPass(RenderParams renderParams, IProfilerWrapper profiler, boolean runningDeferredPass)
	{
		...
		// Cache config values to avoid repeated lookups in render loop
		final boolean cfgEnableGenericRendering = Config.Client.Advanced.Graphics.GenericRendering.enableGenericRendering.get();
		final boolean cfgEnableSsao = Config.Client.Advanced.Graphics.Ssao.enableSsao.get();
		final boolean cfgTransparencyEnabled = Config.Client.Advanced.Graphics.Quality.transparency.get().transparencyEnabled;
		final boolean cfgFadeFarClipPlane = Config.Client.Advanced.Graphics.Quality.dhFadeFarClipPlane.get();
		final boolean cfgEnableDhFog = Config.Client.Advanced.Graphics.Fog.enableDhFog.get();
		final boolean cfgDebugWireframe = Config.Client.Advanced.Debugging.DebugWireframe.enableRendering.get();
		...
		if (!runningDeferredPass)
		{
			...
			if (!deferTransparentRendering && cfgTransparencyEnabled)
			{
