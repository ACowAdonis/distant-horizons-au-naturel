...
public class CloudRenderHandler
{
	...
	// Cached objects for shouldCloudBeCulled() to avoid per-call allocations
	// Safe to reuse because the method is synchronized
	private final Vec3d[] cachedCorners = new Vec3d[] {
		new Vec3d(0, 0, 0),
		new Vec3d(0, 0, 0),
		new Vec3d(0, 0, 0),
		new Vec3d(0, 0, 0)
	};
	private final Vec3d cachedCornerNoHeight = new Vec3d(0, 0, 0);
	private final Vec3d cachedCameraPosNoHeight = new Vec3d(0, 0, 0);
	private final Vec3f cachedToCorner = new Vec3f(0, 0, 0);
			...
			CloudParams cloudParams)
	{
		...
		// using cached array to avoid per-call allocations (safe because method is synchronized)
		this.cachedCorners[0].set(minPosX, minPosY, minPosZ);
		this.cachedCorners[1].set(minPosX, minPosY, minPosZ + cloudParams.widthInBlocks);
		this.cachedCorners[2].set(minPosX + cloudParams.widthInBlocks, minPosY, minPosZ);
		this.cachedCorners[3].set(minPosX + cloudParams.widthInBlocks, minPosY, minPosZ + cloudParams.widthInBlocks);
		...
		// cache camera position without height once before the loop
		this.cachedCameraPosNoHeight.set(cameraPos.x, 0, cameraPos.z);

		for (Vec3d corner : this.cachedCorners)
		{
			// Check if the corner is within the render distance
			// (ignoring height, since LODs also ignore height)
			this.cachedCornerNoHeight.set(corner.x, 0, corner.z);

			double cornerDistance = this.cachedCornerNoHeight.getDistance(this.cachedCameraPosNoHeight);
			...
			this.cachedToCorner.set(
					(float) (corner.x - cameraPos.x),
					(float) (corner.y - cameraPos.y),
					(float) (corner.z - cameraPos.z));
			this.cachedToCorner.normalize();

			if (cameraLookAtVector.dotProduct(this.cachedToCorner) > 0)
			{
