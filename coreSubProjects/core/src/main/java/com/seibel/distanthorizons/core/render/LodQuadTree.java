...
public class LodQuadTree extends QuadTree<LodRenderSection> implements IDebugRenderable, AutoCloseable
{
	...
	// Pooled collections to avoid per-tick allocations (reduces GC pressure)
	private final HashSet<LodRenderSection> nodesNeedingRetrieval = new HashSet<>();
	private final HashSet<LodRenderSection> nodesNeedingLoading = new HashSet<>();
	...
	private void updateAllRenderSections(DhBlockPos2D playerPos)
	{
		...
		// Clear pooled collections for reuse (avoids per-tick allocations)
		this.nodesNeedingRetrieval.clear();
		this.nodesNeedingLoading.clear();
		...
		while (rootPosIterator.hasNext())
		{
			...
			this.recursivelyUpdateRenderSectionNode(playerPos, rootNode, rootNode, rootNode.sectionPos, false, this.nodesNeedingRetrieval, this.nodesNeedingLoading);
		}


		// queue full data retrieval (world gen) requests if needed
		if (this.nodesNeedingRetrieval.size() != 0
		...
		this.loadQueuedSections(playerPos, this.nodesNeedingLoading);
