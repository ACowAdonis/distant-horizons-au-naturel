...
import com.seibel.distanthorizons.core.generation.LodVerificationService;
...
public class DhClientLevel extends AbstractDhLevel implements IDhClientLevel
{
	...
	/** Opportunistic LOD verification service that scans for and recovers missing LODs */
	@Nullable
	private final LodVerificationService lodVerificationService;
		...
		) throws SQLException, IOException
	{
		...
		// Initialize the LOD verification service for opportunistic gap recovery
		this.lodVerificationService = new LodVerificationService(this, this.remoteDataSourceProvider.repo);
	...
	public void clientTick()
	{
		try
		{
			...
			// Run LOD verification when the chunk update queue is idle
			// This opportunistically scans for and recovers missing LODs
			if (this.lodVerificationService != null && SharedApi.INSTANCE.isChunkUpdateQueueIdle())
			{
				this.lodVerificationService.tick();
			}
		}
		catch (Exception e)
		{
