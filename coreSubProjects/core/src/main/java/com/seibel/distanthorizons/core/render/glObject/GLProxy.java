...
public class GLProxy
{
	...
	{
		// Only capture stack trace in dev builds - getStackTrace() is expensive
		// IS_DEV_BUILD is a compile-time constant, so JIT eliminates the dead branch in production
		...
		{
			this.renderThreadRunnableQueue.add(() -> this.runOpenGlCall(renderCall, null));
		}
	}
	private void runOpenGlCall(Runnable renderCall, StackTraceElement[] callerStackTrace)
	{
		try
		{
			renderCall.run();
		...
		catch (Exception e)
		{
			...
			{
				// Use the caller's stack trace for better debugging
				error.setStackTrace(callerStackTrace);
			}
			// If callerStackTrace is null, the exception keeps its natural stack trace
