...
public final class BatchGenerationEnvironment implements IBatchGeneratorEnvironmentWrapper
{
	...
	static
	{
		...
		// SURFACE, CARVERS, LIQUID_CARVERS, and FEATURES generation steps have been removed.
	...
	public BatchGenerationEnvironment(IDhServerLevel dhServerLevel)
	{
		...
		if (!isMcGenerator)
		{
			...
			if (generatorClassName.equals("com.terraforged.mod.chunk.TFChunkGenerator"))
			{
			...
			else if (generatorClassName.equals("net.dries007.tfc.world.TFCChunkGenerator"))
			{
	...
	public void generateEvent(GenerationEvent genEvent) throws RejectedExecutionException
	{
		...
		try
		{
			...
			while (iterator.hasNext())
			{
				...
				// With PRE_EXISTING_ONLY and INTERNAL_SERVER modes, chunks are always complete
				// (either loaded from disk or fully generated by MC)
			...
			DhLitWorldGenRegion region) throws InterruptedException
	{
		...
		try
		{
			...
			// OPTIMIZATION: For pre-existing chunks (generated via Chunky or natural gameplay),
			// biomes are already initialized in the chunk data, so we can skip this step entirely.
			// This provides ~5-10% performance improvement in PRE_EXISTING_ONLY mode.
			if (genEvent.generatorMode == EDhApiDistantGeneratorMode.PRE_EXISTING_ONLY)
			{
				// Skip biomes step - pre-existing chunks already have biome data
				return;
			}
			...
			// SURFACE and FEATURES generation modes removed - only PRE_EXISTING_ONLY and INTERNAL_SERVER are supported
		}
		finally
		{
			...
			// Remove ALL chunks from ignored set, not just successfully generated ones.
			// This ensures positions aren't permanently stuck if generation fails.
