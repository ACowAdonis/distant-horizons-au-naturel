...
public class ChunkWrapper implements IChunkWrapper
{
	...
	public void createDhHeightMaps()
	{
		...
		// Hoist ThreadLocal lookup outside all loops to avoid thousands of redundant lookups per chunk
		...
		for (int x = 0; x < LodUtil.CHUNK_WIDTH; x++)
		{
			for (int z = 0; z < LodUtil.CHUNK_WIDTH; z++)
			{
				...
				blockPos.set(x, y, z);
				IBlockStateWrapper block = BlockStateWrapper.fromBlockState(this.chunk.getBlockState(blockPos), this.wrappedLevel);
